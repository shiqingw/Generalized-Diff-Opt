...
if __name__ == '__main__':
    ...
    cons = [cp.log_sum_exp(kappa_obs*(A_obs_np @ _p + b_obs_np)-x_max_np) - np.log(n_cons_obs) + 1 + x_max_np<= _alpha,
    ...
    p = sp.Matrix(p_vars)
    exps = [sp.exp(kappa_obs*(A_obs_np[i,0] * px + A_obs_np[i,1] * py + b_obs_np[i]) - x_max_np) for i in range(n_cons_obs)]
    con1 = 0
    for exp in exps: con1 += exp
    con1 = con1/n_cons_obs
    con1 = sp.log(con1) + 1 + x_max_np - np.log(n_cons_obs)
    ...
    states[0,:] = x_traj[0,:] + np.array([0,1.5,0,0,0,0]) # initial state
