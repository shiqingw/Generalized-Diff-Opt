...
cons = [cp.log_sum_exp(kappa*(_A1 @ _p - _b1)) <= cp.log(nc1*_alpha),
        cp.log_sum_exp(kappa*(_A2 @ _p - _b2)) <= cp.log(nc2*_alpha)]
# cons = [cp.sum(cp.exp(kappa*(_A1 @ _p - _b1))) <= nc1*_alpha,
#         cp.sum(cp.exp(kappa*(_A2 @ _p - _b2))) <= nc2*_alpha]
...
for i in range(M):
        ...
        _A1.value = to_np(A1_val)
        _b1.value = to_np(b1_val)
        _A2.value = to_np(A2_val)
        _b2.value = to_np(b2_val)
        time6 = time.time()

        problem.solve(solver=cp.SCS, verbose=False, requires_grad=True)
        time7 = time.time()

        _A1.gradient = _A1.value
        _b1.gradient = _b1.value
        _A2.gradient = _A2.value
        _b2.gradient = _b2.value
        problem.backward()
        time8 = time.time()
        ...
        print('Time taken to set the parameters to cvxpy: ', time6 - time5)
        print('Time taken to solve the problem with cvxpy: ', time7 - time6)
        print('Time taken to get the gradient with cvxpy: ', time8 - time7)
        print("")
